...
export function EditBoatListingPage({ onNavigate, boatId }: EditBoatListingPageProps) {
  ...
  useEffect(() => {
    const loadBoat = async () => {
      try {
        setLoading(true);
        const boatData = await boatService.getBoatById(boatId);
        ...
        setBasicInfo({
          ...
          capacity: boatData.capacity.toString(),
          cabins: boatData.cabins.toString(),
        });
        ...
        setDetails({
          price: boatData.price.toString(),
          description: boatData.description,
          image: boatData.image,
        });

        setEquipment(boatData.equipment || []);
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Erreur lors du chargement du bateau');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!boat) {
        throw new Error('Données du bateau manquantes');
      }
      ...
      const updatedBoatData = {
        ...
        destination: location.destination || undefined,
      ...
      setError(err instanceof Error ? err.message : 'Erreur lors de la mise à jour de l\'annonce');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <FormProgress
          currentStep={step}
          totalSteps={5}
          labels={['Infos', 'Lieu', 'Prix', 'Équip.', 'Récap.']}
        />
          ...
          {step === 1 && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-gray-600">Modifiez les caractéristiques principales de votre bateau</p>
              </div>
          ...
          {step === 2 && (
            <div className="space-y-6">
              <div>
                ...
                <h3 className="text-gray-900 mb-2">Localisation</h3>
                <p className="text-gray-600">Indiquez où se trouve votre bateau</p>
              </div>
          ...
          {step === 3 && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-gray-600">Modifiez votre tarif et description</p>
              </div>
              ...
              <p className="text-sm text-gray-500">
                Utilisez des URLs depuis Unsplash ou d'autres sources fiables.
              </p>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-gray-600">
                  Mettez à jour les équipements présents sur votre bateau
                </p>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between border-b border-gray-200 pb-2">
                    ...
                    <span className="text-gray-900">
                      {boatTypes.find(t => t.value === basicInfo.type)?.label}
                    </span>
                  </div>
                  <div className="flex justify-between border-b border-gray-200 pb-2">
